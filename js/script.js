...
// Glow trung tâm
// Thay thế centralGlow

const centralGlow = createGlowMaterial('rgba(255, 150, 180, 0.8)', 156, 0.3); // Màu hồng đỏ, mờ hơn
centralGlow.scale.set(7, 7, 1); // Tăng kích thước một chút
...
const galaxyParameters = {
    ...
    randomness: 0.2,
    randomnessPower: 20,
    insideColor: new THREE.Color(0xff99cc), // Hồng phấn
    outsideColor: new THREE.Color(0xffe6f0), // Hồng nhạt
};
...
function createNeonTexture(image, size) {
    ...
    ctx.closePath();
    ctx.clip();
    ctx.filter = 'brightness(0.95) contrast(1.1) saturate(1.3)'; // Aumenta saturación y contraste
...
for (let group = 0; group < numGroups; group++) {
    ...
    img.onload = () => {
        ...
        // Material khi ở gần
        // Material cuando está cerca
const materialNear = new THREE.PointsMaterial({
    size: 2,
    map: neonTexture,
    transparent: true, // Cambia a true para permitir control de opacidad
    alphaTest: 0.2,
    depthWrite: true,
    depthTest: true,
    blending: THREE.NormalBlending, // Mantén NormalBlending para colores más naturales
    vertexColors: false, // Desactiva vertexColors para evitar que los colores blancos dominen
    opacity: 5 // Reduce la opacidad para un efecto menos brillante
});

// Material cuando está lejos
const materialFar = new THREE.PointsMaterial({
    ...
    alphaTest: 0.2,
    depthWrite: false,
    blending: THREE.NormalBlending, // Cambia a NormalBlending en lugar de AdditiveBlending
    vertexColors: false, // Desactiva vertexColors
    opacity: 5 // Menor opacidad para reducir el brillo
});
...
function createPlanetTexture(size = 512) {
    ...
    // Nền gradient với tông màu lãng mạn
    const gradient = ctx.createRadialGradient(size / 2, size / 2, size / 8, size / 2, size / 2, size / 2);
    gradient.addColorStop(0.00, '#ffccd5'); // Hồng phấn nhẹ
    gradient.addColorStop(0.12, '#ff99aa'); // Hồng đậm hơn
    gradient.addColorStop(0.22, '#ff6688'); // Đỏ hồng
    gradient.addColorStop(0.35, '#ffffff'); // Giữ trắng cho điểm sáng
    gradient.addColorStop(0.50, '#ffb3cc'); // Hồng phấn
    gradient.addColorStop(0.62, '#ff6699'); // Đỏ hồng nhạt
    gradient.addColorStop(0.75, '#ff99cc'); // Hồng pastel
    gradient.addColorStop(1.00, '#ffccdd'); // Hồng nhạt cuối
    ctx.fillStyle = gradient;
    ctx.fillRect(0, 0, size, size);

    // Các đốm màu lãng mạn
    const spotColors = ['#ffccd5', '#ff99aa', '#ff6688', '#ffffff', '#ffb3cc', '#ff6699', '#ff99cc'];
    for (let i = 0; i < 40; i++) {
    ...
    // Các đường cong (swirls) mềm mại hơn
    for (let i = 0; i < 8; i++) {
        ctx.beginPath();
        ...
        ctx.strokeStyle = 'rgba(255, 150, 180, ' + (0.15 + Math.random() * 0.2) + ')'; // Màu hồng nhạt
...
const ringTexts = window.dataLove2Loveloom && window.dataLove2Loveloom.data.ringTexts && window.dataLove2Loveloom.data.ringTexts.length > 0
    ? window.dataLove2Loveloom.data.ringTexts
    : ['ME ENCANTAS TANTO', 'SOLO TÚ IMPORTAS.'];

function createTextRings() {
    const numRings = ringTexts.length;
    const baseRingRadius = planetRadius * 1.04;
    const ringSpacing = 5;
    window.textRings = [];

    for (let i = 0; i < numRings; i++) {
        const ringRadius = baseRingRadius + i * ringSpacing;
        let ringMaterial, ringGeometry;

        if (ringTexts[i % ringTexts.length] === 'Eres la mejor') {
            // Create a bright, opaque ring for "TE AMOO"
            ringGeometry = new THREE.TorusGeometry(ringRadius, 0.3, 16, 128);
            ...
            // Add pink glow sprite
            const glowSprite = createGlowMaterial('rgba(255, 153, 204, 0.8)', 128, 0.8); // Pink glow
            glowSprite.scale.set(ringRadius * 2.2, ringRadius * 2.2, 1); // Slightly larger than ring
            glowSprite.position.set(0, 0, 0);
            textRingMesh.add(glowSprite); // Add glow as child of ring
            ...
            // Existing code for text rings (I LOVE YOU, etc.)
            ...
            let scaleParams = { fontScale: 0.55, spacingScale: 0.9 };
            if (cjkRatio > 0) {
            ...
            const fontSize = Math.max(200, 0.9 * textureHeight);
            ...
            ringGeometry = new THREE.CylinderGeometry(ringRadius, ringRadius, 1.5, 128, 1, true);
            ringMaterial = new THREE.MeshBasicMaterial({
                map: ringTexture,
                transparent: true,
                side: THREE.DoubleSide,
...
function animatePlanetSystem() {
    if (window.textRings) {
        ...
        window.textRings.forEach((ringGroup, index) => {
            ...
            if (textMesh && textMesh.material && textMesh.material.opacity !== undefined) {
                textMesh.material.opacity = 0.7 + pulse * 0.3;
            }
            // Pulse the glow sprite for TE AMOO ring
            if (ringTexts[index % ringTexts.length] === 'TE AMOO' && textMesh.children[0] && textMesh.children[0].material) {
                textMesh.children[0].material.opacity = 0.7 + pulse * 0.2;
            }
        });
...
function createHintText() {
    ...
    const context = canvas.getContext('2d');
    const fontSize = 80;
    const text = window.dataLove2Loveloom && window.dataLove2Loveloom.data.name ? window.dataLove2Loveloom.data.name : 'Kimberly';
    context.font = `bold ${fontSize}px 'Dancing Script', cursive`; // Font lãng mạn
    context.textAlign = 'center';
    context.textBaseline = 'middle';
    context.shadowColor = '#ff99cc'; // Hồng phấn
    context.shadowBlur = 5;
    context.lineWidth = 2;
    context.strokeStyle = 'rgba(255, 0, 72, 5.8)'; // Đỏ hồng mờ
    context.strokeText(text, canvasSize / 2, canvasSize / 2);
    context.shadowColor = '#ff6699'; // Đỏ hồng đậm hơn
    context.shadowBlur = 50;
    context.strokeStyle = 'rgba(255, 200, 220, 0.5)';
    context.strokeText(text, canvasSize / 2, canvasSize / 2);
...
function startCameraAnimation() {
    ...
    function animatePath() {
        ...
        if (progress < duration1) {
            ...
            // Mostrar instrucciones de controles táctiles
            const controlsHint = document.getElementById('controls-hint');
            if (controlsHint) {
                controlsHint.classList.add('visible');
                setTimeout(() => {
                    controlsHint.classList.add('hiding');
                }, 5000);
            }
            return;
        }
